...
5. You can also include `<style></style>` or `<script></script>` tags in here and template that too, but there is a better way, because the `&` symbol doesn't work correctly at times.
